...
| **Template scaffolding & seeding** | `tools/msc-cli/`, `templates/` | `--target` sibling paths only; seed fallback `.sandbox/` | `npm run msc:template -- list|apply|seed|doctor`; plan (archived): [templates-and-msc-cli.md](../../_archive/old-plans/templates-and-msc-cli.md) |
...
| `npm run grade` | v2.6.1 / release audit | `msc-grade-boilerplate.mjs` → structural PASS/FAIL vs TRUTH + Code-Jedi layout (exit **1** on any fail; **61/61**) |
...
### Bootstrap, sandboxes & quality (v2.6.1)
...
### Scaffolding Engine Routines (v2.4.0)
...
*Maintained as the primary AI routing compass for Vader Engine v2.6.1. Update when `package.json` scripts or core module paths change.*
