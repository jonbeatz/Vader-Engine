        ...
        {step === "credentials" && (
          <div>
            ...
            <button
              type="button"
              onClick={() =>
                setCredentials((prev) => [...prev, { label: "", username: "", password: "", visible: false }])
              }
            >
              + Add credential
            </button>
...
export function MscMemberCluster({ members, stackLimit = 3 }: MscMemberClusterProps) {
  ...
  return (
    <div className="msc-member-cluster" style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
      ...
      <details>
        <summary style={{ cursor: "pointer", fontSize: "0.75rem", color: "var(--msc-text-secondary)" }}>
        ...
        <ul style={{ listStyle: "none", padding: 0, margin: "0.5rem 0 0" }}>
          {members.map((m) => (
            <li key={String(m.id)} style={{ fontSize: "0.8rem", marginBottom: "0.35rem" }}>
              {msc_projectMemberLabel(m)}
              {m.email ? (
                <span style={{ color: "var(--msc-text-secondary)", marginLeft: "0.35rem" }}>{m.email}</span>
              ) : null}
            </li>
          ))}
        </ul>
  ...
  return (
    <div className="msc-dashboard-container msc-viewport-shield">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
        ...
        {onAddProject ? (
          <button type="button" onClick={onAddProject} style={{ background: "var(--msc-accent)", color: "#fff", border: "none", padding: "0.5rem 1rem", borderRadius: "var(--msc-radius)" }}>
            + Add Project
          </button>
        ) : null}
      </div>
          ...
          >
            {p.thumbnailUrl ? (
              <img
                src={p.thumbnailUrl}
                alt=""
                style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover", borderRadius: "var(--msc-radius)", marginBottom: "0.75rem" }}
              />
            ) : null}
