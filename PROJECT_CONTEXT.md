...
**Vader Engine** is a Cursor-native, lean full-stack **development factory** (v2.7.0-Engine): structural self-grader (61 checks), triple Next.js sandboxes, Payload CMS example, Tailwind/shadcn Path B sandbox, Studio Dark Shield UI, template scaffolding CLI, portable custom-scriptz modules, interactive backup, and MCP-ready agent workflows.

| Field | Value |
|-------|--------|
| **Version** | `2.7.0` (`vader-engine` in root `package.json`) |
...
| **Active feature work** | Portable modules + backup on `Vader-Engine-Dev-v2` · v2.6.1 on `main` until merge |
...
| **Dashboard** | v2.6.1 on `main` — TanStack Query, 7 API routes, ops hub; v2.7.0 adds portable modules + backup — [v0-integration-roadmap.md](.cursor/plans/v0-integration-roadmap.md) |
