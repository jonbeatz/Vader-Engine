# 🪐 Gold Master Blueprint: Vader Engine Technical Optimization Runbook (Final Update)

**Target:** `Vader Engine-v1` → v2.1.0+ (Jedi-Master baseline)  
...
```bash
cp -r Vader Engine-v1 "../Vader Engine-v1-backup-$(date +%Y%m%d-%H%M)"
```

On Windows PowerShell:

```powershell
Copy-Item -Recurse -Force "Vader Engine-v1" "..\Vader Engine-v1-backup-$(Get-Date -Format 'yyyyMMdd-HHmm')"
```
...
- The operator must choose between conflicting approaches (e.g., repo rename `Vader Engine-v1` → `Vader Engine`)
...
|---------|---------|------------------|
| GitHub repo | `Vader Engine-v1` | Keep or rename to `Vader Engine`; update all URLs consistently |
| README H1 | `Vader Engine — Jedi-Master Edition` | Match repo name + semver in subtitle |
| `package.json` name | `vader-engine` | `msc-boilerplate-v2` |
| `package.json` version | `1.0.0` | `2.1.0` (post-upgrade tag) |

Use `npm run msc:forge -- README.md "Vader Engine-v1" "Vader Engine" --dry-run` before live mutations. The forge shield blocks mutations when `searchStr` matches `/(?:\.msc-|^msc:|^msc_)/i`.
...
- Repo rename automation (`Vader Engine-v1` → `Vader Engine`)
