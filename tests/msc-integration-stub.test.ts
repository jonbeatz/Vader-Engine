...
import { MSC_MEDIA_ENGINE_MODULES, msc_createVideoTrackingEvent } from '../core/msc-core-engine.ts';
import { MSC_PROJECT_ROOT, msc_envHydrationReady } from '../scripts/lib/msc-load-env.mjs';
...
async function msc_runStubAssertions() {
  ...
  if (!report.hydration) {
    throw new Error(
      '[msc:integration-stub] env hydration not ready — check .env.local / .env.example',
    );
  }
  if (!report.engine) {
