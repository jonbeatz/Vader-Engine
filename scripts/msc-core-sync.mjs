...
function getMediaConfig() {
  ...
  return {
    ...
    cdnLocalCacheDir: path.resolve(ROOT, process.env.MSC_CDN_LOCAL_CACHE_DIR?.trim() || 'media'),
  };
}
...
async function main() {
  ...
  if (dbNames) {
    ...
    console.log(
      '[msc:core-sync] filesystem-only mode (pass --with-payload when Payload app is linked)',
    );
  }
