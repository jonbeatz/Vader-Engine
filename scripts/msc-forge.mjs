...
function processFileMutation(filePath, searchStr, replaceStr) {
  if (protectedPathRegex.test(filePath)) {
    console.log(`\x1b[31m[SHIELD ACTIVE] Prevented mutation on protected path: ${filePath}\x1b[0m`);
    process.exit(1);
  }
  ...
  if (protectedNamespaceRegex.test(searchStr) || protectedNamespaceRegex.test(filePath)) {
    console.log(
      `\x1b[31m[SHIELD ACTIVE] Prevented mutation on protected target: ${filePath}\x1b[0m`,
    );
    process.exit(1);
  }
