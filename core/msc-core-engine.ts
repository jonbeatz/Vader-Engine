...
export const MSC_MEDIA_ENGINE_MODULES: MscMediaEngineModule[] = [
  ...
  },
  {
    ...
    description: 'Featured video, gallery grid, and category taxonomy bindings for page layouts.',
  ...
  },
  {
    ...
    description: 'Static CSS generation, theme builder cache integration, and CDN purge triggers.',
  ...
  },
  {
    ...
    description: 'Streaming API auth, video library sync, and pull zone configuration.',
  ...
  },
  {
    ...
    description: 'Click interception, Presto API player init, and fullscreen overlay transitions.',
...
export function msc_buildVideoEmbedUrl(source: MscVideoSource): string {
  ...
  switch (provider) {
    ...
    case 'bunny':
      return (
        streamUrl ??
        `https://iframe.mediadelivery.net/embed/${source.libraryId ?? 'YOUR_LIBRARY_ID'}/${encodeURIComponent(id)}`
      );
    case 'presto':
...
export const MSC_VIDEO_PIPELINE_STEPS = [
  ...
  },
  {
    ...
    description: 'Viewer click activates the player API with a branded fullscreen experience.',
  },
  {
    number: '04',
    title: 'Dynamic Field Mapping',
    description: 'ACF or Payload relationships pull featured/gallery video data into layouts.',
  },
  {
    number: '05',
    title: 'Engine Optimization',
    description: 'Cache management, lazy loading, and CDN routing preserve page performance.',
  },
] as const;
