...
export type MscAuthActionResult =
  | { ok: true }
...
export type MscAuthMailAdapter = (
  to: string,
  subject: string,
  html: string,
) => Promise<boolean>
...
export function msc_validateNewPassword(
  ...
  if (p.length < MSC_PASSWORD_MIN_LENGTH) {
    return { ok: false, message: `Password must be at least ${MSC_PASSWORD_MIN_LENGTH} characters.` }
  }
  if (!/[A-Z]/.test(p)) {
...
export function msc_validateEmail(raw: string): { ok: true; email: string } | { ok: false; error: string } {
...
export function msc_buildVerificationEmailHtml(options: {
  ...
  const paragraphs = (options.bodyParagraphs ?? [
