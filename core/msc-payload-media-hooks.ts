...
export function msc_getMediaEngineConfig(cwd = process.cwd()): MscMediaEngineConfig {
  ...
  return {
    ...
    localMediaRoot: path.resolve(
      cwd,
      process.env.MSC_MEDIA_LOCAL_DIR?.trim() || "media",
    ),
    tenantUploadsRoot: path.resolve(
      cwd,
      process.env.MSC_MEDIA_TENANT_ROOT?.trim() || "uploads",
    ),
    ...
    cdnPublicBaseUrl:
      process.env.MSC_CDN_PUBLIC_BASE_URL?.trim() || "https://cdn.your-domain.com",
...
export function msc_payloadSharpConfig(config?: MscMediaEngineConfig): { sharp?: SharpConstructor } {
...
export function msc_buildCdnStorageStub(
  config?: MscMediaEngineConfig,
): MscCdnStorageAdapterStub {
  const cfg = config ?? msc_getMediaEngineConfig()
  return {
    kind: "stream-cdn",
...
export function msc_ensureMediaAlt(
...
export function msc_toRelativePublicMediaUrl(
  input: string,
  config?: MscMediaEngineConfig,
): string {
...
export function msc_buildMediaAccess(options: {
  ...
  const hasAdmin = options.hasAdminAccess ?? ((role: MscMediaOwnerRole) =>
...
export function msc_defaultMediaHooks(options: {
  tenantOwner?: boolean
  hasAdminAccess?: (role: MscMediaOwnerRole) => boolean
  config?: MscMediaEngineConfig
  ...
  if (options.tenantOwner) {
    hooks.beforeChange = [
      ...(hooks.beforeChange ?? []),
      msc_mediaOwnerBeforeChangeHook(hasAdmin),
    ]
  }
...
export function msc_buildMediaCollection(
  ...
  const staticDir =
    options.staticDir ?? msc_resolveMediaStaticDir(cfg, tenantId)
